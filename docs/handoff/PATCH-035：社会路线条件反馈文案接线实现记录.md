...
- 规则视图始终公开冷却正文模板和 `next_available_day` 的参数来源；只在当前确实处于冷却时输出已代入日期的当前事实正文。
- 终章炉律或路线行动因设施未运行而拒绝时，按路线返回守炉堂或巡查所条件正文。
- 悼亡钟继续公开结构化 `requires_recorded_death` 及当前是否满足，不输出与现行累计死亡机制不符的“近期死亡”正文。
- 留城劝诫和需要熟食的行动分别返回与真实失败原因一致的条件正文。
- `rules_view("oath_order")` 提前公开上述确认、冷却和行动条件，供机器在提交前发现；不提供行动推荐或路线选择建议。

## 4. 测试

- 两条路线缺少首次确认时返回同一互斥正文，且不出现旧 `confirm sign` 写法；
- 无冷却时规则视图不伪造“第 1 天”当前事实，有冷却时动态插入真实下一可签署日期；
- 守炉堂、巡查所分别返回正确设施正文；
- 悼亡钟仅返回结构化累计死亡条件，历史死亡可合法使用；旧城派未激活和熟食不足分别返回正确正文；
- 所有拒绝保持状态不变；
...
- `requirement.death_recent` 的“近期死亡”正文与现行累计死亡条件不一致，按用户确认撤出运行注册；正式时间窗口或新玩家正文待确认，本轮不新增时间机制。
