...
`survival.json` 保存已封存的普通难度开局值、基础食耗、炉心档位、固定天气与区域温度修正。Patch 004 新增 `buildings.json`，保存建筑目录、区域槽位、升级、`heat` 与用户允许先行测试的 `woodfuel` 数值；该文件声明为 `TEST_NUMERIC`，不表示所有平衡值已经最终封存。正式运行配置必须：
