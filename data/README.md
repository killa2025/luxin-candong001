...
`survival.json` 保存已封存的普通难度开局值、基础食耗、炉心档位、固定天气与区域温度修正。`buildings.json` 保存建筑目录、地表资源点、区域槽位、升级、`heat` 与测试态 `woodfuel` 数值。Patch 005 新增 `laws.json`，保存 006A 炉律关系、配给、工时、医疗与社会行动规则；测试数值声明为 `TEST_NUMERIC`，未封存的分诊数值不进入运行配置。正式运行配置必须：
