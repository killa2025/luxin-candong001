...
`survival.json` 保存已封存的普通难度开局值、基础食耗、炉心档位、固定天气与区域温度修正。`buildings.json` 保存建筑目录、地表资源点、区域槽位、升级、`heat` 与测试态 `woodfuel` 数值。`maps.json` 保存三张 V1 地图、随机权重、共享小型资源点/猎区/森林数量与大型煤钢矿点差异。`laws.json` 保存 006A 炉律关系、配给、工时、医疗与社会行动规则。Patch 006 新增 `technologies.json`，保存 37 项 006B 科技、研究成本与天数、单队列研究规则、第二研究所倍率和过载压力规则；测试数值声明为 `TEST_NUMERIC`，未封存科技效果只登记为 `DEFERRED`，不伪装成运行效果。正式运行配置必须：
