...
代码 Patch 020 已完成复审并并入 `main`，其终局主文案使用本局种子稳定选择，重复查看不会重抽。当前 Patch 021 为 D49～D55 每日日结补充可验证的食堂与医疗服务历史：记录食堂是否实际运行、实际运行医疗建筑数和有效建筑医疗容量，并据此安全接回 `ending.additional.food.01`、`ending.additional.medical.01` 与 `.02`。旧档缺失的历史只标记为未知，不从终局建筑反推；009-C 完整路线长文、正式随机权重、图形终局页与 D56 仍保持 PENDING，本补丁不修改任何平衡数值或游戏机制。
...
- 当前存档数据版本为 v16。v15 → v16 为既有终霜每日记录加入明确的“服务历史未知”字段，不从终局建筑倒推过去；新结算日保存实际服务事实。v14 → v15 的终局报告格式迁移及此前 v13 → v14 的饥饿、寒冷、木材供应与终霜迁移语义保持不变。
