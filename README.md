...
代码 Patch 018 已完成复审并并入 `main`。当前 Patch 019 依据同种子、同地图的誓言/铁腕盲玩证据，按“偏好结局需要稳定规划，但不制造必死局”的保守口径进行第二轮社会平衡测试：火盆集会每日被动减压不能把恐慌压到 20 以下；守炉宣誓、悼亡钟和留城劝诫的重复纠错能力进一步收紧，留城劝诫新增固定熟食成本；承诺跨日兑现另写结构化日志。所有变更继续属于 `TEST_NUMERIC`，不代表最终封存平衡值；铁腕路线、医疗、建筑生产、科技、地图资源与第七霜落数值均未修改。
