...
代码 Patch 026～031 及各自复审修正均已并入 `main`；Patch 030 已通过第二十八次纯黑盒验收，Patch 031 已完成复审并合并。Patch 032 只接入用户确认的南方来信、暗中传言、公开集结与离城倒计时正文，并把既有封存标题、选项文字接入旧城机器视图；不修改旧城触发、阶段、选项效果、人数、资源、信任、恐慌或其他机制与平衡。旧城实际离开或资源损失不符合现有终局正文时只登记 PENDING；`sedation_city` 正式触发公式尚未封存，其正文暂不进入运行选择。
