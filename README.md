...
- 当前存档数据版本为 v11；除 Patch 008 的旧城派与 006C 状态外，还保存 D49 基线、终霜每日记录、D55 六项评分、总分与分级标签。v10 → v11 只接受 D49 之前的可重建存档；D49 及以后因无法确定性补造终霜历史而拒绝迁移。若 v10 存档曾接纳固定增员，则从该增员首个压力窗口日已经结算起拒绝迁移，因为 v10 不保存可确定重建的五日压力历史；拒绝增员的存档不受此限制。
