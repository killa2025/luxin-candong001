...
当前仓库正在实施代码 Patch 013：在既有确定性日结上补齐 D1～D55 寒冷暴露、四层饥饿、饥饿疾病/重症/死亡与社会压力、D49 木材供应链锁死，以及更保守的额外医疗配给试玩数值。所有新增平衡值均保持 `TEST_NUMERIC`，仍需盲玩复核；未封存的大型矿机数值、身份级人口交叉表、候选池条件、死亡记录句和图形界面继续保持 PENDING。
...
- 当前存档数据版本为 v14；在 v13 地图状态之外，新增四层饥饿及整数余数、寒冷暴露余数、绑定结算日的无房人口寒冷快照、D49 木材供应快照、全局/终霜饥饿统计和终局限制因素。v13 → v14 不伪造旧局未曾保存的未进食人数；已有终霜历史继续按 v13 食物评分语义校验，并显式标记历史人数未知。
