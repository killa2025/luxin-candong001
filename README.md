...
代码 Patch 021 已完成复审并并入 `main`。当前 Patch 022 根据三次 Patch 021 黑盒局进行第二阶段难度校准：两次纯盲玩分别在 D52、D53 失败，一次目标导向谨慎局在 D55 取得 14 分苦涩胜利，因此本轮不增加伤病、不减少地图资源，只把新局的 D49“准备充分”要求从 5/8 收紧到 6/8，并提高煤粮、信任、恐慌及五档终局门槛；高胜还要求六系统均至少 3 分且历史死亡比例不超过 5%。这些值继续保持 `TEST_NUMERIC`，目标是偏难但非死局，不是最终封存平衡。
