...
代码 Patch 015 已并入 `main`。当前 Patch 016 修复第五次盲玩发现的 D48→D49 强制停工状态延迟，并把已封存的三次霜落预警正文接入机器事件视图；终局报告会明确标出正文是否仍有待定文本，学徒路线会明确区分“模式已解锁”与“人口生成公式尚未封存”。誓言、铁腕、医疗、住房、资源产出和终霜强度等平衡数值均未在本补丁调整。
