...
代码 Patch 019 已完成复审并并入 `main`。当前 Patch 020 接通第六轮已封存的终局主文案、叙事报告句、重大状态附加句、一句式路线痕迹和执政官拷问池，并加入用户本轮确认的无死亡、墓园、冷藏坑、未处理遗体与余烬名册死亡记录句。文案先按真实终局状态筛选，再由本局随机种子作稳定选择；重复查看、存档读取和回放不会重抽。009-C 完整路线长文、正式随机权重、图形终局页与 D56 仍保持 PENDING，本补丁不修改任何平衡数值或游戏机制。
...
- 终局报告保存实际选中的 `text_id` 和报告格式版本；Patch 020 使用本局种子对符合状态的封存候选文案作稳定选择，并严格校验结局、标签、具体死亡事实与所选文案一致。既有 v14 的 Patch 010 部分报告迁移为格式 1 并原样读取，不在加载时重抽；新报告固定使用格式 2，不能替换成旧报告形状绕过校验。
...
- 当前存档数据版本为 v15。v14 → v15 只增加明确的终局报告格式版本：已生成的 Patch 010 旧报告标记为格式 1，未生成报告和 Patch 020 新报告使用格式 2。此前 v13 → v14 的饥饿、寒冷、木材供应与终霜迁移语义保持不变。
