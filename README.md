...
代码 Patch 026～047 及各自复审修正均已并入 `main`。Patch 048 在新局试行科研人手规则（`TEST_NUMERIC`），不修改地图、产出、医疗或社会数值。历史测试结果保留在实现记录中，不在玩家入口提供旧局经验。寒冷与终局仍使用 `patch045` 平衡档；新局另使用 `staffing_patch048` 科研档。正式机器标准流为 UTF-8；旧记录和已生成报告不在加载时自动改分或重抽文案。
...
- 当前存档数据版本为 v19。v18 → v19 保留旧研究进度和全部历史，并添加 `legacy_fixed` 科研档与零小数余数；旧局继续使用原研究速度。新局为 `staffing_patch048` 科研档。寒冷与终局的 `patch045` 档不变：高胜最低 24 分，社会系统 4 分要求信任至少 85、恐慌至多 15。v17 → v18 保留旧局的 `legacy_patch021` 或 `patch022` 档。终局报告格式仍为 6，已生成的格式 1～6 报告原样严格读取，不改分、不重抽。更早迁移语义不变。

## 科研人手试行规则

寒冷与评分口径未变：同等级、同住房状态的人口会先汇总再计算寒冷患病，高胜最低总分为 24；不因科研人手试行而调整其他系统。

新局每座研究所 10 名正式工程师达到完整基础速度（4 研究点/日）；5 人为 2 点，1 人为 0.4 点，0 人不推进。学徒额外科研贡献尚未确认，不计入本轮速度。每座最多 10 个岗位、全城最多两所、唯一研究队列保持不变。

两所分别按实际工程师投入计算；加班只放大目标研究所的贡献，再取较大贡献全额、较小贡献一半。两所满员通常为 6 点/日，只有一所加班时为 8 点/日。一所满员加班为 6 点/日。

科研在既有研究阶段读取当时可运行研究所的合法工程师人数（医疗、死亡等此前阶段先结算）。十分之一点精确积累并随存档保存；无人工作时保留已有进度，完成或取消研究时清空全部余量，不转给下一项科技。科技的名义研究天数表示工作量，不保证当前人手下的完成日期。

`observe().research_view`、`status().research.speed_contract` 与 `rules(technologies).interface_text.research_speed` 公开同源科研档、公式、当前贡献、小数进度和读取阶段。这是当前状态下的能力说明，不保证日结前疾病或人员变化之后仍有同样进度，不提供行动推荐。旧局明确显示 `legacy_fixed`，仍使用原固定速度与全队列加班方式。
