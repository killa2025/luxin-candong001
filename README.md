...
代码 Patch 016 已完成复审并并入 `main`。当前 Patch 017 修复第六次盲玩确认的固定增员医疗风险预览陈旧值，并接入已经封存的儿童请求、固定增员标题/选项/提示和第七霜落标题；非法命令格式会返回稳定的合法请求示例。仓储按用户明确确认口径在生产阶段开始时判定：已满或超仓会停止普通地表采集、建筑生产和会增加仓储占用的食堂加工；当日产出越过容量时全部保留，下一生产日再停产，固定增员与事件资源仍可超仓，任何情况下都不删除或截断资源。誓言、铁腕、医疗效果和其他平衡数值均未调整。
