...
代码 Patch 026、Patch 027、Patch 027 合并后黑盒修复及 Patch 028 均已完成复审并并入 `main`；Patch 028 合并后诊断修复已经第二十六次纯黑盒验收通过。Patch 029 正在接入用户明确确认的四段终局报告正文，只补齐无运行医疗服务与煤炭/食物见底分支，不修改机制或平衡。
...
- 终局报告保存实际选中的 `text_id` 和报告格式版本；Patch 020 使用本局种子对符合状态的封存候选文案作稳定选择，Patch 021 再用同日服务快照证明医疗与食堂经历。Patch 027 的格式 3 报告以 D55 服务事实和终局实际库存过滤主报告句；Patch 029 的格式 4 报告为这些事实分支接入用户确认正文。既有格式 1 / 2 / 3 报告均按各自合同原样严格读取，不在加载时重抽，也不能用其他格式形状绕过校验。
...
- 当前存档数据版本为 v17。v16 → v17 只增加终霜平衡档标识：既有存档保留 `legacy_patch021` 档，新建局使用 `patch022` 档，避免新门槛重写旧局结论。Patch 020～026 形成、尚未生成终局报告的格式 2 进行中存档可继续加载，并在正式终局时生成当前格式 4 报告；已经生成的格式 1 / 2 / 3 报告保持原样。v15 → v16 的逐日服务历史迁移明确只处理历史格式 2 报告，v14 → v15 的报告格式迁移及此前迁移语义保持不变。
