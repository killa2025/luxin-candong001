...
代码 Patch 022 已完成复审并并入 `main`。当前独立修复依据 Patch 022 的 seed 22022 黑盒局处理两项确定问题：实际炉级为 0 的整夜停炉若当日没有其他自然死亡，则由既有寒冷系统保证至少 1 人冻死；生产阶段从未满仓跨入超仓时保留全部产出，并在同一次成功日结立即返回结构化超仓警告。Patch 022 的 D49 与终局门槛继续保持 `TEST_NUMERIC`，目标仍是偏难但非死局。
...
- `game.set_furnace` 在白天设置最终炉心档位 `0～3`；炉心关闭或燃料不足会在 `end_day` 前产生结构化强警告。若日结实际有效炉级为 0 且疾病、饥饿尚未造成当日自然死亡，既有寒冷系统保证至少 1 人冻死；警告会明确返回该最低后果。
