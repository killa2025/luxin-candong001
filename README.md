...
代码 Patch 017 已完成复审并并入 `main`。当前 Patch 018 依据第四至第七次盲玩证据，按用户确认的暂行测试口径进行第一轮社会平衡复核：火盆集会每日被动减压不能把恐慌压到 10 以下；誓言路线减少无条件签署收益，延长守炉宣誓与共食誓餐冷却、降低其情绪收益，并提高终火誓约的恐慌代价。所有变更继续属于 `TEST_NUMERIC`，不代表最终封存平衡值；铁腕路线、医疗配给、第二研究所、建筑产出、科技成本、地图资源与第七霜落数值均未修改。
...
- `data/laws.json` 保存 006A 炉律关系、配给、工时、医疗与社会行动规则，包括已登记为 `TEST_NUMERIC` 的过劳患病公式、事故风险点、Patch 013 额外医疗配给 5 食物/人、最多 10 人、冷却 5 天，以及 Patch 018 火盆集会每日被动恐慌下限 10；未封存的分诊结果及事故结果不进入运行配置。
...
- `data/oath_order.json` 保存代码 Patch 008 的旧城派阈值、006C 解锁、炉律关系、行动成本与冷却；Patch 018 只覆盖用户确认的誓言路线暂行测试值，铁腕值保持不变，全部继续标记为 `TEST_NUMERIC`。
