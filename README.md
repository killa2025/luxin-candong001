...
代码 Patch 026～034 及各自复审修正均已并入 `main`；Patch 030、Patch 032、Patch 033、Patch 034 已分别通过第二十八次至第三十一次纯黑盒验收，Patch 031 已完成事件正文接线。Patch 035 只接入已封存的 7 条路线互斥、设施运行、炉约冷却、旧城派与熟食条件反馈；悼亡钟继续公开结构化累计死亡条件，不输出与机制不符的“近期死亡”正文，所有社会路线机制与平衡不变。旧城实际离开或资源损失不符合现有终局正文时只登记 PENDING；`sedation_city` 正式触发公式尚未封存，其正文暂不进入运行选择。
