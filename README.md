...
代码 Patch 026、Patch 027 及 Patch 027 合并后黑盒修复均已完成复审并并入 `main`；后者也通过旧格式 2 存档的 D52～D55 专项回归。Patch 028 收口自动存档快照的机器接口：主存档继续作为 `play` / `report` 的可恢复状态，日期推进前的 `autosave_end_day` 快照通过独立只读包络查看，直接误用时返回明确文件角色与恢复说明。
...
- 当前存档数据版本为 v17。v16 → v17 只增加终霜平衡档标识：既有存档保留 `legacy_patch021` 档，新建局使用 `patch022` 档，避免新门槛重写旧局结论。Patch 020～026 形成、尚未生成终局报告的格式 2 进行中存档可继续加载，并在正式终局时生成格式 3 报告；v15 → v16 的逐日服务历史迁移明确只处理历史格式 2 报告，v14 → v15 的报告格式迁移及此前迁移语义保持不变。
