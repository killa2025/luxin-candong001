...
代码 Patch 026～029 及各自复审修正均已并入 `main`；Patch 029 已通过第二十七次纯黑盒验收。Patch 030 正在接入用户确认的路线、旧城派、儿童制度与娱乐制度终局长文，只补齐可由正式状态直接证明的叙事分支，不修改机制或平衡。旧城实际离开或资源损失不符合现有正文时只登记 PENDING；`sedation_city` 正式触发公式尚未封存，其正文暂不进入运行选择。
