...
代码 Patch 014 已并入 `main`，日结食物强预警会在状态副本上复用正式建筑生产和食堂加工逻辑。当前 Patch 015 修复盲玩发现的配给预览与实际结算不一致，并补充 woodfuel 零贡献原因、配给降级原因、不可逆钢材供应锁死、D49 强制停工、炉律/路线效果及人员参数语义等机器反馈。誓言、铁腕、医疗、住房、资源产出和终霜强度等平衡数值均未在本补丁调整。
