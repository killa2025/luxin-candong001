...
代码 Patch 026～032 及各自复审修正均已并入 `main`；Patch 030 已通过第二十八次纯黑盒验收，Patch 031 已完成事件正文接线，Patch 032 已通过第二十九次纯黑盒验收。Patch 033 只接入用户确认的事件选项不可用提示、同类承诺重复提示及承诺成功/失败标题，并将其接到正式事件面板、拒绝结果和承诺结算日志；不修改事件或承诺机制与平衡。旧城实际离开或资源损失不符合现有终局正文时只登记 PENDING；`sedation_city` 正式触发公式尚未封存，其正文暂不进入运行选择。
