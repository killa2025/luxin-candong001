...
当前仓库已完成代码 Patch 004：在 Patch 001～003 的机器接口、固定 `end_day` 编排和基础生存状态之上，接入建筑目录、地表资源采集、区域槽位、手动建造、逐座即时升级、岗位分配、`heat`、`woodfuel`、建筑温度与基础生产。部分岗位产出使用可存档的精确整数余数累计，不使用浮点数。完整疾病与死亡、炉律签署、科技研究、事件、承诺及终局仍按后续 Patch 分批实现。
